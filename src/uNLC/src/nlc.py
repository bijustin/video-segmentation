...
def imresize(old_image, frac):
    ...
    size = tuple((np.array(im.size) * frac).astype(int))
...
def compute_saliency(imSeq, flowSz=100, flowBdd=12.5, flowF=3, flowWinSz=10,
    ...
    for i in range(n):
        temp_img = Image.fromarray(imSeq[i])
        size = tuple((flowSz, flowSz))
        im[i] = np.array(temp_img.resize(size, PIL.Image.BICUBIC))
...
def nlc(imSeq, maxsp, iters, outdir=None, suffix='',
